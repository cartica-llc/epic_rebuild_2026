...
export async function GET(req: NextRequest) {
    ...
    const projectNumber = searchParams
        .get('projectNumber')
        ?.trim()
        .toUpperCase();
    ...
    if (!DB || !SCHEMA) {
        return NextResponse.json(
            { error: 'Snowflake database/schema env vars are missing' },
            { status: 500 },
        );
    }
    ...
    try {
      ...
      WHERE UPPER(PROJECT_NUMBER) = '${projectNumber.replace(/'/g, "''")}'
    `;
        ...
        const rows = (await query(sql)) as { CNT: number | string }[];
        ...
        return NextResponse.json(
            { error: 'Failed to check project number' },
            { status: 500 },
        );
    }
}