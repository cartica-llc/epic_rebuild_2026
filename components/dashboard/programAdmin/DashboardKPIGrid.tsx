// components/dashboard/program_admin/DashboardKPIGrid.tsx
...
function KPICard({ label, value, sub, accent }: KPICardProps) {
    return (
        <div className={`rounded-2xl p-5 shadow-sm ring-1 ${accent ? 'bg-amber-50 ring-amber-200/70' : 'bg-white ring-slate-200/70'}`}>
...
export function DashboardKPIGrid({ kpis }: { kpis: DashboardKPIs }) {
    const burnRate = kpis.totalCommittedFunding > 0
    ...
    return (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
            ...
            <KPICard
                ...
                value={formatCurrency(kpis.totalCommittedFunding)}
            />
            <KPICard
                label="Spent to Date"
