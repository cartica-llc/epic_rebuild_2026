...
import { Search, X, ChevronDown, ArrowRight, BarChart3 } from 'lucide-react';
...
export function Insight_TechnologySearch() {
    ...
    const investmentAreas = React.useMemo(
        () => Array.from(new Set(projects.flatMap((p) => splitMultiValue(p.INVESTMENT_AREAS)))).sort(),
        [],
    );

    const cpucProceedings = React.useMemo(
        () => Array.from(new Set(projects.flatMap((p) => splitMultiValue(p.CPUC_PROCEEDINGS)))).sort(),
        [],
    );

    const statuses = React.useMemo(
        () => Array.from(new Set(projects.map((p) => p.PROJECT_STATUS).filter(Boolean))).sort(),
        [],
    );
    ...
    const filteredProjects = React.useMemo(() => {
        ...
        return projects.filter((p) => {
            ...
            return !(selectedStatus && p.PROJECT_STATUS !== selectedStatus);
        });
                    ...
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                        <div>
                            ...
                            <p className="mt-1.5 max-w-lg text-[13px] leading-relaxed text-slate-500">
                                Search similar EPIC projects and preview summaries, innovations, learnings, and barriers.
                            </p>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {STARTER_TOPICS.map((topic) => (
                                <button
                                    ...
                                    type="button"
                                    onClick={() =>
                                        setSearchTerm(searchTerm.toLowerCase() === topic.toLowerCase() ? '' : topic)
                                    }
                        ...
                        <div className="flex flex-wrap gap-2">
                            {LENS_OPTIONS.map((item) => (
                                <button
                                    key={item.key}
                                    type="button"
                                    onClick={() => setSelectedLens(selectedLens === item.key ? null : item.key)}
                                    className={`rounded-md px-4 py-2.5 text-sm font-medium transition ${
                                        selectedLens === item.key
                                            ? 'bg-slate-900 text-white shadow-sm'
                                            : 'border border-slate-200 bg-white text-slate-600 shadow-sm hover:text-slate-900'
                                    }`}
                                >
                                        ...
                                        {[
                                            {
                                                ...
                                                options: investmentAreas,
                                            },
                                            {
                                                label: 'CPUC Proceeding',
                                                value: selectedProceeding,
                                                onChange: setSelectedProceeding,
                                                options: cpucProceedings,
                                            },
                                            {
                                            ...
                                            <div key={f.label}>
                                                ...
                                                <div className="relative">
                                                    <select
                                                        ...
                                                        className="w-full appearance-none rounded-md border border-slate-200 bg-white px-3 py-2.5 pr-8 text-sm text-slate-700 shadow-sm outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
                                                    >
                ...
                <div className="space-y-5 px-4 py-5 md:px-6 md:py-6">
                    <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                        ...
                        <div className="rounded-md border border-slate-200 bg-white p-4">
                            ...
                            <p className="mt-2 text-xl font-semibold text-slate-900">
                                {LENS_OPTIONS.find((l) => l.key === selectedLens)?.label ?? 'Best available'}
                            </p>
                        </div>
                        <div className="rounded-md border border-slate-200 bg-white p-4">
                            <p className="text-[11px] uppercase tracking-wide text-slate-500">Filters Applied</p>
                            <p className="mt-2 text-xl font-semibold text-slate-900">
                                {[selectedArea, selectedProceeding, selectedStatus].filter(Boolean).length}
                            </p>
                        </div>
                    </div>
                    ...
                    {filteredProjects.length === 0 ? (
                        <div className="rounded-md border border-slate-200 bg-white p-10 text-center">
                            ...
                            <p className="text-sm text-slate-500">
                                {projects.length === 0
                                    ? 'Connect Snowflake data to populate results.'
                                ...
                                >
                                    ...
                                    <div className="flex flex-wrap gap-x-5 gap-y-1 text-xs text-slate-500">
                                        <span>
                                            ...
                                            {project.PROJECT_LEAD || 'Not listed'}
                                        </span>
                                        <span>
                                            <span className="font-medium text-slate-700">Committed:</span>{' '}
                                            {formatFunding(project.COMMITTED_FUNDING_AMT)}
                                        </span>
                                    </div>
                                    <div className="mt-3">
                                        <button
