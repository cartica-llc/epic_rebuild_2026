...
function getAreas(p: EpicProject) {
    return p.INVESTMENT_AREAS.split(',').map((s) => s.trim());
}

function primaryArea(p: EpicProject) {
    return getAreas(p)[0];
}

function colorFor(area: string) {
    return INNOVATION_COLORS[area] ?? DEFAULT_COLOR;
}
...
function allUniqueAreas(projects: EpicProject[]) {
    ...
    projects.forEach((p) => {
        getAreas(p).forEach((a) => set.add(a));
    });
...
export function Insight_LocationInsights() {
    ...
    useEffect(() => {
        fetch('https://cdn.jsdelivr.net/npm/us-atlas@3/states-10m.json')
            .then((r) => r.json())
            .then((us: Topology) => {
                const states = topojson.feature(us, us.objects.states) as FeatureCollection;
    ...
    const filtered = useMemo(() => {
        ...
        if (activeArea) {
            list = list.filter((p) => getAreas(p).includes(activeArea));
        }

        if (showDACLI) {
            list = list.filter((p) => p.CPUC_DACLI === 1);
        }
    ...
    const spending = useMemo(() => {
        ...
        filtered.forEach((p) => {
            acc.committed += p.COMMITTED_FUNDING_AMT;
            acc.contracted += p.CONTRACTED_FUNDING_AMT;
            acc.expended += p.EXPENDED_FUNDING_AMT;
            acc.count += 1;
        });
        ...
        <div className="bg-white">
                    ...
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                        <div>
                            ...
                            <p className="mt-1.5 max-w-lg text-[13px] leading-relaxed text-slate-500">
                                Explore EPIC project locations across California.
                            </p>
                        </div>
                        ...
                        {hasFilters && (
                            <button
                                ...
                                className="self-start rounded-md border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-500 shadow-sm hover:text-slate-900"
                            >
                                Reset filters
                            </button>
                        )}
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <select
                                    value={activeArea ?? ''}
                                    onChange={(e) => {
                                        setActiveArea(e.target.value || null);
                                        setSelectedId(null);
                                    }}
                                    className="w-full min-w-[220px] appearance-none rounded-md border border-slate-200 bg-white px-3 py-2.5 pr-8 text-sm text-slate-700 shadow-sm outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
                                >
                                    <option value="">All Areas</option>
                                    {areas.map((area) => (
                                        <option key={area} value={area}>
                                            {colorFor(area).label || area}
                                        </option>
                                    ))}
                                </select>
                        ...
                        <button
                            ...
                            className={`rounded-md px-4 py-2.5 text-sm font-medium transition ${
                                showDACLI
                                    ...
                                    : 'border border-slate-200 bg-white text-slate-600 shadow-sm hover:text-slate-900'
                            }`}
                        >
                            DAC / LI
                        </button>
            ...
            <div className="space-y-5 px-4 py-5 md:px-6 md:py-6">
                ...
                <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                    {[
                        ['Projects in View', spending.count],
                        ['Committed', spending.committed],
                        ['Contracted', spending.contracted],
                        ['Expended', spending.expended],
                    ].map(([label, val]) => (
                        <div key={label} className="rounded-md border border-slate-200 bg-white p-4">
                            <p className="text-[11px] uppercase tracking-wide text-slate-500">{label}</p>
                            <p className="mt-2 text-xl font-semibold text-slate-900">
                                {typeof val === 'number' && label !== 'Projects in View' ? fmt(val) : val}
                            </p>
                        </div>
                    ))}
                </div>
                ...
                <section className="rounded-md border border-slate-200 bg-white p-4 md:p-5">
                    <div className="mb-4">
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                            {filtered.length} project{filtered.length !== 1 ? 's' : ''} · California
                        </p>
                    </div>
                            ...
                            <svg viewBox="0 0 380 480" className="block h-auto w-full">
                                ...
                                {filtered.map((p) => (
                                    <circle
                                        ...
                                        onClick={() => {
                                            setSelectedId(
                                                selectedId === p.PROJECT_ID ? null : p.PROJECT_ID,
                                            );
                                        }}
                                    />
                                ))}
                        ...
                        {epicProjects.length === 0 && (
                            <div className="absolute inset-0 flex items-center justify-center">
                                <p className="text-xs text-slate-400">
                                    Connect Snowflake data to plot projects
                                </p>
                            </div>
                        )}
                    </div>
                    ...
                    {filtered.length === 0 ? (
                        <div className="py-10 text-center">
                            <p className="mb-1 text-sm font-semibold text-slate-900">
                                {epicProjects.length === 0 ? 'No data loaded' : 'No projects match'}
                            </p>
                            <p className="text-sm text-slate-500">
                                {epicProjects.length === 0
                                    ? 'Connect Snowflake to populate.'
                        ...
                        <div className="space-y-2">
                            {filtered.map((p) => (
                                <div
                                    ...
                                    className="cursor-pointer rounded-md border border-slate-200 bg-white p-3 transition hover:border-slate-300 hover:bg-slate-50/60"
                                >
                                    ...
                                    <div className="mt-1.5 flex flex-wrap items-center gap-1.5 text-[11px] text-slate-500">
                                        <span
                                            className="inline-block h-2 w-2 rounded-full"
                                            style={{ background: colorFor(primaryArea(p)).dot }}
                                        />
