...
function PNStatus({ tone, children }: { tone: 'error' | 'warn' | 'muted'; children: React.ReactNode }) {
    const cls = {
...
// ── Visibility toggle ──
function VisibilityToggle({ active, onToggle }: { active: boolean; onToggle: () => void }) {
    return (
        <div className="w-full flex items-start gap-4 rounded-xl border border-slate-200 bg-slate-50 px-5 py-4">
            ...
            <div className="min-w-0">
                <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-slate-800">{active ? 'Active' : 'Inactive'}</span>
                    ...
                    }`}>{active ? 'Published' : 'Unpublished'}</span>
                </div>
...
export function ProjectTab({ data, set, lookups, isMaster, lockedPrefix, onAddCompany, requiredKeys, mode, projectId }: {
    ...
    const pnCheck = useProjectNumberCheck(prefix, isEdit ? '' : data.projectNumber, isEdit ? projectId : undefined);

    const numberPartMax = prefix ? Math.max(0, PROJECT_NUMBER_MAX - prefix.length - 1) : PROJECT_NUMBER_MAX;
    const fullNumberLength = prefix ? prefix.length + 1 + data.projectNumber.length : data.projectNumber.length;
    const tooLong = fullNumberLength > PROJECT_NUMBER_MAX;

    const trackDeleted = (img?: { name: string; isExisting?: boolean } | null) => {
        if (img?.isExisting) set('deletedImages', [...(data.deletedImages || []), img.name]);
    };
    ...
    return (
        <div className="flex flex-wrap gap-x-5 gap-y-4">
            ...
            <Field label="Project number" required={req('projectNumber')}>
                ...
                {isEdit && <p className="mt-1 text-[11px] text-slate-400">Project numbers cannot be changed after creation</p>}
                {!isEdit && tooLong && (
                    <PNStatus tone="error">
                ...
                {!isEdit && !tooLong && pnCheck.status === 'taken' && (
                    <PNStatus tone="error"><XCircle size={12} className="shrink-0" /> This project number already exists — please enter a different number</PNStatus>
                )}
                {!isEdit && !tooLong && pnCheck.status === 'checking' && (
                    <PNStatus tone="muted"><Loader2 size={12} className="shrink-0 animate-spin" /> Verifying project number…</PNStatus>
                )}
                {!isEdit && !tooLong && pnCheck.status === 'error' && (
                    <PNStatus tone="warn"><Info size={12} className="shrink-0" /> Could not verify — please try again</PNStatus>
                )}
                {!isEdit && !tooLong && pnCheck.status !== 'checking' && pnCheck.status !== 'taken' && pnCheck.status !== 'error' && prefix && data.projectNumber && (
                    <p className="mt-1 text-[11px] text-slate-400">{fullNumberLength}/{PROJECT_NUMBER_MAX} characters</p>
                )}
            </Field>
            ...
            <Field label="Project status" tooltip="Whether the project is still active or has been completed." span="full" required={req('projectStatus')}>
            ...
            <Field label="Project public URL"><TextInput placeholder="https://..." value={data.projectPublicUrl} onChange={(e) => set('projectPublicUrl', e.target.value)} /></Field>
            <Field label="Project website URL"><TextInput placeholder="https://..." value={data.projectWebsiteUrl} onChange={(e) => set('projectWebsiteUrl', e.target.value)} /></Field>

            <div className="w-full sm:w-[calc(50%-10px)]">
                <ImageUploadZone label="Main Project Image" images={data.mainImage ? [data.mainImage] : []}
                                 onAdd={(img) => set('mainImage', img)}
                                 onRemove={() => { trackDeleted(data.mainImage); set('mainImage', null); }}
            ...
            <div className="w-full sm:w-[calc(50%-10px)]">
                <ImageUploadZone label="Gallery Images" images={data.galleryImages || []}
                                 ...
                                 onRemove={(i) => {
                                     ...
                                     set('galleryImages', (data.galleryImages || []).filter((_, idx) => idx !== i));
                                 }}
            ...
            <VisibilityToggle active={data.isActive} onToggle={() => set('isActive', !data.isActive)} />
            ...
            <div className="hidden w-full flex items-start gap-2 rounded-lg bg-slate-50 border border-slate-200 px-3 py-2.5 text-xs text-slate-500">
            ...
            <Field label="Project lead" required={req('leadCompanyId')}>
                <CompanySearchSelect value={data.leadCompanyId} onChange={(id) => set('leadCompanyId', id)} companies={lookups?.companies ?? []} onAddNew={onAddCompany} placeholder="Search lead company..." />
            </Field>
            <Field label="Partners">
                <CompanyMultiSelect value={data.partnerCompanyIds} onChange={(ids) => set('partnerCompanyIds', ids)} companies={lookups?.companies ?? []} placeholder="Search partner companies..." />
            </Field>
            ...
            {String(data.programAdminId) === '0' && (
                <>
                    ...
                    <Field label="CEC manager email" span="full" required={req('cecMgrEmail')}><TextInput type="email" placeholder="email@energy.ca.gov" value={data.cecMgrEmail} onChange={(e) => set('cecMgrEmail', e.target.value)} /></Field>
                </>
            )}
        </div>
    );
